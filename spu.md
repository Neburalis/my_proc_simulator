...
- **Оперативная память**: 1024 ячейки типа `double`.
  - Используется как **универсальное хранилище**, в том числе как **текстовая видеопамять** (например, для команды `DRAW`).
  - Команды `POPM` и `PUSHM` записывают и читают значения как `double` без каких-либо преобразований.
  - При выводе кадра (`DRAW`) каждая ячейка приводится к `char`, что позволяет хранить символы (например, `65.0` для `'A'`) и одновременно работать с числовыми значениями иного масштаба.
...
- `POPM [<reg>]` — извлечь значение из стека и записать его как `double` в память по адресу, указанному в регистре `<reg>`. Значение в памяти будет перезаписано, а старое — утеряно.
- `PUSHM [<reg>]` — прочитать `double` из памяти по адресу из регистра `<reg>` и поместить его в стек. Значение в памяти сохранится (аналог регистров, но не именованных).
...
- Вся оперативная память (`1024 double-ячейки`) может использоваться как **текстовый видеобуфер**.
- Каждая ячейка при выводе приводится к `char`, поэтому значение `65.0` отображается как `'A'`.
